                    ...
                    if !current.isNative {
                        ...
                        Section("Controller") {
                            ...
                            Toggle("Rumble through XInput", isOn: $gameOptions.xinputRumble)
                                .help("For a game that has a controller but no vibration. Some titles drive the pad themselves and rumble it directly; others only know how to rumble through XInput, and a DualSense is not an XInput device, so they stay silent. This offers the pad's motors to XInput as a small device of its own, beside the pad -- the pad itself is not replaced, wrapped or hidden, so its adaptive triggers, touchpad and PlayStation glyphs are exactly as they were. It costs nothing in frames: the motors ride inside a packet the game is already sending, so nothing of ours is added to the Bluetooth link. Titles that use Sony's own library used to refuse to start with this on, because the small device carried the pad's vendor and product ids and that library enumerates by them; it is now offered to XInput alone and one such title was measured starting with this on. TURN IT OFF FOR A GAME YOU PLAY THROUGH STEAM INPUT. Steam already presents a virtual pad that IS an XInput device, and this adds a second one with no sticks on it, which a game can pick instead. Needs the engine controller set from Options, and takes effect when the pad next arrives: start with Steam closed, or reconnect the pad.")
