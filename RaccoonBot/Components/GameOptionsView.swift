                    ...
                    if !current.isNative {
                        ...
                        Section("Controller") {
                            ...
                            Toggle("Rumble through XInput", isOn: $gameOptions.xinputRumble)
                                .help("For a game that has a controller but no vibration. Some titles drive the pad themselves and rumble it directly; others only know how to rumble through XInput, and a DualSense is not an XInput device, so they stay silent. This offers the pad's motors to XInput as a small device of its own, beside the pad -- the pad itself is not replaced, wrapped or hidden, so its adaptive triggers, touchpad and PlayStation glyphs are exactly as they were. It no longer costs frames: the motors now ride inside a packet the game is already sending, so nothing of ours is added to the Bluetooth link. LEAVE IT OFF FOR A GAME THAT USES SONY'S OWN LIBRARY. None of those start with it on -- the small device shares the pad's vendor and product ids, and that library enumerates by them -- and none of them need it, because they drive the pad themselves and already rumble. This is for the games that have never heard of a DualSense. Needs the engine controller set from Options, and takes effect when the pad next arrives: start with Steam closed, or reconnect the pad.")
