...
//        let sharedSupportComponent = "Contents/SharedSupport/Crossover"
//        try f.copyItem(at: sourceCXPath, to: destUrl)
//        let destSharedSupportURL = destUrl.appendingPathComponent(sharedSupportComponent, isDirectory: true)
//        let sharedSupportURL = patchedApp.appendingPathComponent(sharedSupportComponent, isDirectory: true)
//        try f.removeItem(at: destSharedSupportURL)
//        try f.copyItem(at: sharedSupportURL, to: destSharedSupportURL)
        if(f.fileExists(atPath: destUrl.path())) {
            // Step 2 unsign app
