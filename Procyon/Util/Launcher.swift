...
func openSteam(cxAppPath: String?, selectedBottle: String?) {
    ...
    if let bottleName = URL(string: selectedBottle!)?.lastPathComponent {
        let steamLaunchCommand = "MVK_CONFIG_USE_METAL_ARGUMENT_BUFFERS=0 CX_GRAPHICS_BACKEND=\"d3dmetal\" \(cxAppPath!)/Contents/SharedSupport/CrossOver/bin/wine --bottle \(bottleName) \"C:\\Program Files (x86)\\Steam\\Steam.exe\""
        do {
